...
export type GlassSurfaceProps = {
  ...
  borderRadius?: number;
  borderWidth?: number; // as a fraction of min(width, height)
  brightness?: number; // 0-100
  opacity?: number; // 0-1
  blur?: number; // px
  displace?: number; // stdDeviation for feGaussianBlur
...
`;
  ...
  const supportsSVGFilters = () => {
    ...
    // @ts-expect-error backdropFilter is not in CSSStyleDeclaration typings in older TS libs
  ...
  const supportsBackdropFilter = () => {
    if (typeof window === "undefined") return false;
    return (window.CSS && CSS.supports("backdrop-filter", "blur(10px)")) || false;
  };
      ...
      >
        <defs>
          <filter id={filterId} colorInterpolationFilters="sRGB" x="0%" y="0%" width="100%" height="100%">
            {/* @ts-expect-error: ref typing for SVG elements can be finicky across TS versions */}
